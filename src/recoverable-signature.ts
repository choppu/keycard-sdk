...
export class RecoverableSignature {
  ...
  public static recoverFromSignature(recId: number, hash: Uint8Array, r: Uint8Array, s: Uint8Array, compressed: boolean): Uint8Array {
    let signature = new Uint8Array(r.byteLength + s.byteLength + 1);
    signature[0] = recId;
    signature.set(r, 1);
  ...
  fromTLV(hash: Uint8Array, tlvData: Uint8Array): void {
    ...
    Object.assign(this, {publicKey: publicKey, r: r, s: s, compressed: compressed});
  ...
  calculateRecID(hash: Uint8Array): number {
    ...
    for (let i = 0; i < 4; i++) {
      let candidate = RecoverableSignature.recoverFromSignature(i, hash, this.r!, this.s!, this.compressed!);
