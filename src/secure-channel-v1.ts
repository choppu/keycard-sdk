...
export class SecureChannelV1 implements SecureChannel {
  secret!: Uint8Array;
  ...
  async autoPair(apduChannel: CardChannel, pairingMode: number, sharedSecret: Uint8Array) : Promise<void> {
    ...
    let resp = await this.pair(apduChannel, PAIR_P1_FIRST_STEP, pairingMode, challenge);
    ...
    resp = await this.pair(apduChannel, PAIR_P1_LAST_STEP, 0, checkCryptogram);
  ...
  async pair(apduChannel: CardChannel, p1: number, p2: number, data: Uint8Array) : Promise<APDUResponse> {
    let pair = new APDUCommand(0x80, INS_PAIR, p1, p2, data);
